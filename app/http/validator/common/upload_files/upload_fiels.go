...
func (u UpFiles) CheckParams(context *gin.Context) {
	...
	//获取文件发生错误，可能上传了空文件等
	if err != nil {
		response.Fail(context, consts.FilesUploadFailCode, consts.FilesUploadFailMsg, consts.FilesUploadFailMsg, err.Error())
		return
	}
	...
	if tmpFile.Size > variable.ConfigYml.GetInt64("FileUploadSetting.Size")<<20 {
		response.Fail(context, consts.FilesUploadMoreThanMaxSizeCode, consts.FilesUploadMoreThanMaxSizeMsg+variable.ConfigYml.GetString("FileUploadSetting.Size"), consts.FilesUploadMoreThanMaxSizeMsg+variable.ConfigYml.GetString("FileUploadSetting.Size"), "")
		return
	}
	//不允许的文件mime类型
	...
	if !isPass {
		response.Fail(context, consts.FilesUploadMimeTypeFailCode, consts.FilesUploadMimeTypeFailMsg, consts.FilesUploadMimeTypeFailMsg, "")
	} else {
