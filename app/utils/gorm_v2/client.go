...
func GetUserMysqlClient(dbName string) (*gorm.DB, error) {
	sqlType := "Mysql"
	readDbIsOpen := 1
	//readDbIsOpen := variable.ConfigGormv2Yml.GetInt("Gormv2." + sqlType + ".IsOpenReadDb")
	return GetUserSqlDriver(sqlType, readDbIsOpen, dbName)
}
...
// 获取数据库驱动, 可以通过options 动态参数连接任意多个数据库
func GetUserSqlDriver(sqlType string, readDbIsOpen int, dbName string, dbConf ...ConfigParams) (*gorm.DB, error) {

	var dbDialector gorm.Dialector
	if val, err := getUserDbDialector(sqlType, "Write", dbName, dbConf...); err != nil {
		variable.ZapLog.Error(my_errors.ErrorsDialectorDbInitFail+sqlType, zap.Error(err))
	} else {
		dbDialector = val
	}
	...
	// 读写分离配置只
	if readDbIsOpen == 1 {
		if val, err := getUserDbDialector(sqlType, "Read", dbName, dbConf...); err != nil {
			variable.ZapLog.Error(my_errors.ErrorsDialectorDbInitFail+sqlType, zap.Error(err))
		} else {
			dbDialector = val
		}
...
func getUserDbDialector(sqlType, readWrite string, dbName string, dbConf ...ConfigParams) (gorm.Dialector, error) {
	var dbDialector gorm.Dialector
	dsn := getDBUserDsn(sqlType, readWrite, dbName, dbConf...)
	switch strings.ToLower(sqlType) {
	case "mysql":
		dbDialector = mysql.Open(dsn)
	case "sqlserver", "mssql":
...
func getDsn(sqlType, readWrite string, dbConf ...ConfigParams) string {
	...
	switch strings.ToLower(sqlType) {
	case "mysql":
		return fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?charset=%s&parseTime=True&loc=%s", User, Pass, Host, Port, DataBase, Charset, "Asia%2FSingapore")
	case "sqlserver", "mssql":
...
func getDBUserDsn(sqlType, readWrite string, dbName string, dbConf ...ConfigParams) string {
	Host := variable.ConfigGormv2Yml.GetString("Gormv2." + sqlType + "." + readWrite + ".Host")
	DataBase := dbName
	Port := variable.ConfigGormv2Yml.GetInt("Gormv2." + sqlType + "." + readWrite + ".Port")
	User := variable.ConfigGormv2Yml.GetString("Gormv2." + sqlType + "." + readWrite + ".User")
