...
type RiderWalletSummaryStruct struct {
...
func GetRiderMemberBalance(memId string, walletTypeId string) (*RiderWalletSummaryStruct, error) {
	var bal *RiderWalletSummaryStruct
	//now := time.Now().Format(variable.DateFormat)
	query := db.Table("rider_wallet_summary as a").
...
func RiderWalletAdjustment(tx *gorm.DB, memId string, walletTypeId string, amount float64) bool {
	var query *gorm.DB
	//now := time.Now().Format(variable.DateFormat)
	//db := GetDB()
	if amount > 0 {
		query = tx.Table("rider_wallet_summary as a").
			Where("a.member_id = ?", memId).
			Where("a.wallet_type_id = ?", walletTypeId).
...
func CreateRiderMemberWallet(tx *gorm.DB, memId string, walletTypeId string) (*RiderWalletSummaryStruct, error) {
	memWallet := RiderWalletSummaryStruct{
		MemberId:     memId,
	...
	err := tx.Table("rider_wallet_summary").Create(&memWallet).Error
	if err != nil {
		return nil, err
	}
