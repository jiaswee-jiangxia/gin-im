...
func init() {
	...
	var err error
	signKeyByte, err := ioutil.ReadFile("storage/oauth-private.key")
	signKey, err = jwt.ParseRSAPrivateKeyFromPEM(signKeyByte)
	if err != nil {
		log.Fatalf("[initKeys]: %s\n", err)
	}
	verifyKeyByte, err := ioutil.ReadFile("storage/oauth-public.key")
	verifyKey, err = jwt.ParseRSAPublicKeyFromPEM(verifyKeyByte)
	if err != nil {
		log.Fatalf("[initKeys]: %s\n", err)
	}

	variable.PublicKey = verifyKey
	variable.PrivateKey = signKey
	//}
